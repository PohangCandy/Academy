...
public:
	...
	{
		// C++의 rand()를 사용하여 무작위 맵을 생성하는 로직
		// srand(time(0));은 main.cpp 또는 초기화 시 단 한 번만 호출해야 합니다.

		// 맵 초기화: 장애물을 제외한 모든 경로 탐색 흔적과 데이터 초기화
		// InitMap()만으로는 장애물이 그대로 남아있으므로, resetObs()도 사용해야 합니다.
		...
		for (int i = 0; i < obs_count; ++i)
		{
			...
			// Start 또는 Goal 지점은 장애물로 설정하지 않습니다.
			if (CheckTile(y, x) != start && CheckTile(y, x) != goal)
			{
				// 장애물로 설정하면 ChangeTile 내부에서 g, h, f가 0으로 초기화됩니다.
				ChangeTile(y, x, obs);
			}
		}
