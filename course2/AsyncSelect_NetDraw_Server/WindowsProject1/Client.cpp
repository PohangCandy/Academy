...
void ProcessWrite() {
    ...
    //---------------------------------------------------------------------------------------------------
    // 1. send 하는데 모아서 처리하지 않고 바로 헤더 읽은만큼 보내는데 이렇게 하면 성능에 이상없을까?
    // 2. 링버퍼 배열 끝부분에 대한 처리가 제대로 동작하고 있나?
    // 지금 상태라면 링버퍼 끝에서 배열의 첫번째까지 데이터가 걸쳐있는 경우, 
    // window에서 send는 무조건 0이 발생할거임. 이때 사실 보낼 수 있는 데이터가 링버퍼에 담겨있다면
    // 찌꺼기 데이터를 보내는 작업이 필요함.
    //---------------------------------------------------------------------------------------------------
