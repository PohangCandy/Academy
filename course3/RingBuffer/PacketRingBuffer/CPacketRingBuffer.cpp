...
CPacketRingBuffer::CPacketRingBuffer(void)
    : m_packetBuffer(nullptr), m_iBufferSize(0), m_iFront(0), m_iRear(0), m_bIsFull(false)
{
    InitializeCriticalSection(&m_csRingbuffer);
}

CPacketRingBuffer::CPacketRingBuffer(int iBufferSize)
    : m_packetBuffer(new CPacket*[iBufferSize]), m_iBufferSize(iBufferSize), m_iFront(0), m_iRear(0), m_bIsFull(false)
{
    InitializeCriticalSection(&m_csRingbuffer);
}
...
CPacketRingBuffer::~CPacketRingBuffer() {
    if (m_packetBuffer != nullptr) {
        delete[] m_packetBuffer; // 할당된 메모리 해제
...
bool CPacketRingBuffer::Enqueue(CPacket* pPacket)
{
    ...
    m_packetBuffer[m_iRear] = pPacket;

    m_iRear = (m_iRear + 1) % m_iBufferSize;
    //--------------------------------------------------------------------
...
bool CPacketRingBuffer::Dequeue(CPacket*& pPacket)
{
    ...
    pPacket = m_packetBuffer[m_iFront];

    m_iFront = (m_iFront + 1) % m_iBufferSize;
    m_bIsFull = false;
...
CPacket** CPacketRingBuffer::GetFrontBufferPtr(void)
{
    if (m_iBufferSize == 0)
        return nullptr;
    return m_packetBuffer + m_iFront;
}
...
void CPacketRingBuffer::GetLockBuffer()
{
    EnterCriticalSection(&m_csRingbuffer);
}

void CPacketRingBuffer::UnLockBuffer()
{
    LeaveCriticalSection(&m_csRingbuffer);
}
