#include "stdafx.h"
#include "ChattingServer.h"
#include "CPacketForMultiThread.h"
#include "CrashDump.h"
#include "CSystemLog.h"
#include "CConfigParser.h"
#include <conio.h>

// ChattingServer.cpp에 정의된 전역 캐릭터 풀
extern myMemorypool::CMemoryPool<Character> characterpool;

int main()
{
	CCrashDump::Init();

	//------------------------------------------------------------
	// 설정 파일 로드
	//------------------------------------------------------------
	CConfigParser config;
	if (!config.Open("ServerConfig.ini"))
	{
		printf("[ChatServer] ServerConfig.ini load failed!\n");
		return 1;
	}

	// 시스템 로그 설정
	const char* logDir = config.GetString("System", "LOG_DIRECTORY", "Log");
	const char* logLevelStr = config.GetString("System", "LOG_LEVEL", "DEBUG");
	bool consoleOutput = config.GetBool("System", "CONSOLE_OUTPUT", false);

	wchar_t wLogDir[256];
	MultiByteToWideChar(CP_ACP, 0, logDir, -1, wLogDir, 256);
	SYSLOG_DIRECTORY(wLogDir);

	CSystemLog::en_LOG_LEVEL logLevel = CSystemLog::LEVEL_DEBUG;
	if (_stricmp(logLevelStr, "ERROR") == 0)  logLevel = CSystemLog::LEVEL_ERROR;
	if (_stricmp(logLevelStr, "SYSTEM") == 0) logLevel = CSystemLog::LEVEL_SYSTEM;
	SYSLOG_LEVEL(logLevel);
	CSystemLog::GetInstance()->SetConsoleOutput(consoleOutput);

	// 서버 설정
	int chatPort    = config.GetInt("ChatServer", "PORT", 21501);
	int maxSession  = config.GetInt("ChatServer", "MAX_SESSION", 20000);

	// 모니터링 설정
	const char* monitorIP = config.GetString("Monitor", "IP", "127.0.0.1");
	int monitorPort       = config.GetInt("Monitor", "PORT", 20000);
	int serverNo          = config.GetInt("Monitor", "SERVER_NO", 3);

	// 풀 상한 설정
	int maxPacketPool = config.GetInt("Pool", "MAX_PACKET_POOL", 50000);
	int maxCharPool   = config.GetInt("Pool", "MAX_CHAR_POOL", 30000);
	CPacket::packetPool.SetMaxCapacity(maxPacketPool);
	characterpool.SetMaxCapacity(maxCharPool);

	printf("[ChatServer] Config loaded - Port:%d, MaxSession:%d, Monitor:%s:%d\n",
		chatPort, maxSession, monitorIP, monitorPort);
	printf("[ChatServer] Pool caps - PacketPool:%d, CharPool:%d\n",
		maxPacketPool, maxCharPool);

	ChattingServer chatserver;

	// 모니터링 서버에 LAN 접속
	chatserver.ConnectMonitor(monitorIP, monitorPort, serverNo);

	// Start()는 non-blocking (accept가 별도 스레드)
	chatserver.Start(chatPort, maxSession);

	// 메인 스레드 대기 ('q' 키로 Graceful Shutdown)
	while (true)
	{
		if (_kbhit())
		{
			char ch = _getch();
			if (ch == 'q' || ch == 'Q')
				break;
		}
		Sleep(100);
	}

	printf("[ChatServer] Shutting down...\n");
	chatserver.Stop();
	printf("[ChatServer] Shutdown complete\n");

	return 0;
}
