...
CPacketRingBuffer::CPacketRingBuffer(int iBufferSize)
    : m_pBuffer(new CPacket*[iBufferSize]), m_iBufferSize(iBufferSize), m_iFront(0), m_iRear(0), m_bIsFull(false)
{
    InitializeCriticalSection(&m_csRingbuffer);
}
...
//--------------------------------------------------------------------
int CPacketRingBuffer::Dequeue(char* chpDest, int iSize)
{
...
int CPacketRingBuffer::Peek(char* chpDest, int iSize)
{
    if (iSize <= 0 || chpDest == nullptr || m_iBufferSize == 0)
        return 0;
